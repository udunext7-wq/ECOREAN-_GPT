...
export type ViewKey = 'dashboard' | 'project' | 'approvals' | 'risks' | 'ontology' | 'floorplanCenter' | 'aiVisualization' | 'masterDb' | 'estimate' | 'bathroomEstimate' | 'kitchenEstimate' | 'fullRemodelingEstimate' | 'contractDocuments' | 'constructionSchedule' | 'purchaseOrders' | 'executionManagement' | 'ceoControlTower' | 'communication' | 'payment' | 'closing' | 'caseLibrary' | 'costCapture' | 'marginSafety' | 'vendorPrice' | 'portfolio' | 'crew' | 'finance' | 'sales' | 'client' | 'settings' | 'profitTemplates' | 'profitAutomation';
