...
export type ViewKey = 'dashboard' | 'project' | 'approvals' | 'risks' | 'ontology' | 'floorplanCenter' | 'aiVisualization' | 'boardGeneration' | 'lightbimImport' | 'lightbimQuantityReview' | 'lightbimExecutionFeedback' | 'lightbimTraceability' | 'lightbimSpaceMap' | 'lightbimCustomerMap' | 'masterDb' | 'franchise' | 'fieldMobile' | 'clientPortal' | 'analytics' | 'aiAutomation' | 'estimate' | 'bathroomEstimate' | 'kitchenEstimate' | 'fullRemodelingEstimate' | 'contractDocuments' | 'constructionSchedule' | 'purchaseOrders' | 'executionManagement' | 'ceoControlTower' | 'communication' | 'payment' | 'closing' | 'calibration' | 'caseLibrary' | 'costCapture' | 'marginSafety' | 'vendorPrice' | 'vendorIntelligence' | 'portfolio' | 'crew' | 'finance' | 'sales' | 'client' | 'settings' | 'profitTemplates' | 'profitAutomation';
