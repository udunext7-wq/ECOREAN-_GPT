...
동일 현장조사 ID에 주소가 반복 등록된 경우도 중복 후보로 표시합니다.
...
빈 문자열, 숫자만 있는 값, 건물명만 있는 값, `null` 또는 잘못된 payload는 앱을 중단시키지 않고 `UNKNOWN`, `LOW`, `INVALID` 등 안전한 상태로 처리합니다. 공백으로 둘러싼 구분용 하이픈은 제거하되 지번/건물 부번의 하이픈은 유지합니다.
