...
| RC030-S4-003 | S4 | 단가표 일괄 가져오기 | XLSX 직접 파싱은 로컬 파서 의존성이 있을 때만 지원하며 RC-0.3.0의 검증된 경로는 CSV임 | XLSX 파일을 파서 의존성 없이 가져오기 | CSV 운영 경로 유지, XLSX는 후속 검토 | DEFERRED | RC-0.4.0 |
| RC030-S4-004 | S4 | 단가표 일괄 가져오기 | 미매칭 항목의 신규 마스터 자동 생성은 지원하지 않음 | 미등록 자재가 포함된 CSV 가져오기 | 수동 마스터 등록 후 재가져오기 | DEFERRED | RC-0.4.0 |
