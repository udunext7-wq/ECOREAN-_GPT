...
function createCrmPipelineService({ sqliteService, reportsDir, crmNextActionService } = {}) {
  ...
  let nextActionService = crmNextActionService || null;

  function setNextActionService(service) {
    nextActionService = service || null;
  }

  function runAutomation(method, ...args) {
    if (!nextActionService || typeof nextActionService[method] !== 'function') return null;
    return nextActionService[method](...args);
  }
  ...
  function createCrmLead(payload = {}) {
    return withDb((database) => {
      ...
      const result = { ok: true, leadId, lead: getCrmLeadDetailFromDb(database, leadId) };
      result.nextActionAutomation = runAutomation('generateNextActionsForLead', leadId, { source: 'SYSTEM_RULE', assignedTo: normalized.assigned_to });
      return result;
    });
  }
  ...
  function moveCrmStage(leadId, nextStage, payload = {}) {
    ...
    return withDb((database) => {
      ...
      const result = { ok: true, leadId: lead.lead_id, fromStage: lead.stage, toStage: normalizedStage, lead: getCrmLeadDetailFromDb(database, lead.lead_id) };
      result.nextActionAutomation = runAutomation('generateNextActionsForStageChange', lead.lead_id, lead.stage, normalizedStage, {
        source: 'STAGE_CHANGE',
  ...
  function createConsultationLog(leadId, payload = {}) {
    ...
    return withDb((database) => {
      ...
      const result = { ok: true, logId, detail: getCrmLeadDetailFromDb(database, lead.lead_id) };
      const nextAction = clean(args.payload.nextAction || args.payload.next_action);
      if (nextAction) {
        result.nextActionAutomation = runAutomation('createCrmNextAction', {
          leadId: lead.lead_id,
          actionType: 'FOLLOW_UP',
          title: nextAction,
          descriptionInternal: '상담 기록에서 생성된 내부 후속 작업',
          priority: lead.priority,
          dueAt: args.payload.nextActionDueAt || args.payload.next_action_due_at,
          assignedTo: lead.assigned_to,
  ...
  function createSiteSurveyRequest(leadId, payload = {}) {
    ...
    return withDb((database) => {
      ...
      const result = { ok: true, surveyId, detail: getCrmLeadDetailFromDb(database, lead.lead_id) };
      result.nextActionAutomation = runAutomation('generateNextActionsForStageChange', lead.lead_id, lead.stage, 'SITE_SURVEY_SCHEDULED', {
        source: 'SITE_SURVEY',
        assignedTo: args.payload.assignedTo || args.payload.assigned_to || lead.assigned_to,
        requestedDate: args.payload.requestedDate || args.payload.requested_date
      });
      return result;
    });
  }
  ...
  function linkEntity(leadId, field, value, stage, reason) {
    return withDb((database) => {
      ...
      const result = { ok: true, leadId: lead.lead_id, linkedId: entityId, lead: getCrmLeadDetailFromDb(database, lead.lead_id) };
      if (stage && stage !== lead.stage) {
        result.nextActionAutomation = runAutomation('generateNextActionsForStageChange', lead.lead_id, lead.stage, stage, {
          source: field === 'linked_estimate_id' ? 'ESTIMATE_LINK' : 'PROJECT_LINK',
          assignedTo: lead.assigned_to
        });
      }
      return result;
